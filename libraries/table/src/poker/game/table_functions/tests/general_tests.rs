...
fn test_table_new() {
    ...
    assert_eq!(table.big_blind, convert_to_e8s(2.0));
    assert_eq!(table.small_blind, convert_to_e8s(1.0));
...
fn test_add_user() {
    ...
    assert_eq!(
        table.get_player_at_seat(0).unwrap(),
        Principal::from_text("2chl6-4hpzw-vqaaa-aaaaa-c").expect("Could not decode principal")
    );
}
...
fn test_get_blind_user_principals() {
    ...
    assert_eq!(
        table.get_small_blind_user_principal().unwrap(),
        Principal::from_text("br5f7-7uaaa-aaaaa-qaaca-cai").expect("Could not decode principal")
    );
    assert_eq!(
        table.get_big_blind_user_principal().unwrap(),
        Principal::from_text("bw4dl-smaaa-aaaaa-qaacq-cai").expect("Could not decode principal")
    );
...
fn test_folding_transfers_balance() {
    ...
    assert_eq!(user2.balance, convert_to_e8s(102.0));
}
...
fn test_all_in_player_with_side_pot() {
    ...
    println!("Small blind: {:?}", small_blind_uid.to_text());
    println!("Big blind: {:?}", big_blind_uid.to_text());
    println!("Other: {:?}", other_uid.to_text());

    table.users.get_mut(&other_uid).unwrap().balance = convert_to_e8s(120.0);
    table.users.get_mut(&small_blind_uid).unwrap().balance = convert_to_e8s(90.0);
