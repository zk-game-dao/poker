...
impl ClanIndex {
    ...
    pub fn search_by_tags(&self, filters: TagSearchFilters) -> Vec<ClanId> {
        ...
        if let Some(categories) = filters.categories {
            let category_clan_ids: HashSet<ClanId> = self
                .clans
                .iter()
                .filter(|(_, clan)| {
                    clan.tags
                        .iter()
                        .any(|tag| categories.contains(&tag.category()))
                })
                .map(|(id, _)| *id)
                .collect();

            result_ids = result_ids
                .intersection(&category_clan_ids)
                .copied()
                .collect();
        }
...
pub fn validate_clan_tags(tags: &HashSet<ClanTag>) -> Result<(), String> {
    ...
    let skill_tags: Vec<&ClanTag> = tags
        .iter()
