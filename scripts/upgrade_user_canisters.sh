...
WASM_PATH="target/wasm32-unknown-unknown/release/user_canister.wasm"
...
echo "About to upgrade ${#USER_CANISTER_IDS[@]} canisters with wasm from: $WASM_PATH"
echo ""
...
total_canisters=${#USER_CANISTER_IDS[@]}
current_canister=0
successful_upgrades=0
failed_upgrades=0

# Loop over each canister ID and perform the upgrade
for CANISTER_ID in "${USER_CANISTER_IDS[@]}"
do
    ((current_canister++))
